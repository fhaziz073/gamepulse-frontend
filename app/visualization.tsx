...
import { applyStatBasisToSeriesList, StatBasis } from "@/utils/statBasis";
...
export default function App() {
  ...
  const [primaryEntity, setPrimaryEntity] = useState<EntitySearchResult>(
    PLAYER_DEFAULTS.primary,
  );
  const [secondaryEntity, setSecondaryEntity] = useState<EntitySearchResult>(
    PLAYER_DEFAULTS.secondary,
  );
  const [searchTarget, setSearchTarget] = useState<"primary" | "secondary">(
    "primary",
  );
  ...
  const [appliedFilters, setAppliedFilters] =
    useState<AppliedFilters>(defaultRange);
  ...
  const [singleSelectedIndex, setSingleSelectedIndex] = useState<number | null>(
    null,
  );
  const [compareSelectedIndex, setCompareSelectedIndex] = useState<
    number | null
  >(null);
  ...
  const [opponentFilterTeam, setOpponentFilterTeam] =
  ...
  const apiBaseUrl = link;
  const availableStats = useMemo(
    () => (mode === "team" ? TEAM_STATS : PLAYER_STATS),
    [mode],
  );
  ...
  const loadChart = useCallback(async () => {
    ...
    const ids = compareOn
      ? [primaryEntity.id, secondaryEntity.id]
      : [primaryEntity.id];
  ...
  const chartResponse = useMemo((): VisualizationResponse | null => {
    ...
    return {
      ...lastResponse,
      seriesList: applyStatBasisToSeriesList(
        lastResponse.seriesList,
        statBasis,
      )!,
    };
  ...
  const compareChartHasAbsence = useMemo(() => {
    if (
      !compareOn ||
      !lastResponse?.seriesList ||
      lastResponse.seriesList.length < 2
    )
      return false;
    const pts = [
      ...lastResponse.seriesList[0].points,
      ...lastResponse.seriesList[1].points,
    ];
  ...
  const teamH2hHasAbsence = useMemo(() => {
    if (
      ...
      return false;
    return [
      ...chartResponse.seriesList[0].points,
      ...chartResponse.seriesList[1].points,
    ].some((p) => Boolean(p.chartAbsence));
  ...
  const chartRangeLabel = useMemo(() => {
    if (appliedFilters.startDate || appliedFilters.endDate) {
      const startText = appliedFilters.startDate
        ? formatRangeDate(appliedFilters.startDate)
        : "Earliest";
      const endText = appliedFilters.endDate
        ? formatRangeDate(appliedFilters.endDate)
        : "Latest";
  ...
  const applyPresetRange = (
    preset: "pastMonth" | "past3Months" | "thisSeason",
  ) => {
    const nextRange =
      preset === "pastMonth"
      ...
      <View style={styles.mainShell}>
        ...
        >
          <ScrollView
            ...
            contentContainerStyle={[
              ...
              {
                paddingBottom:
                  Math.max(insets.bottom, SCREEN_PADDING) + sheetCollapsedPad,
            ...
            <View style={styles.toggleRow}>
              ...
              >
                <Text
                  style={[
                    styles.pillText,
                    mode === "player" && styles.pillTextActive,
                  ]}
                >
                  Player
                </Text>
              ...
              >
                <Text
                  style={[
                    styles.pillText,
                    mode === "team" && styles.pillTextActive,
                  ]}
                >
                  Team
                </Text>
              ...
              <View style={styles.toggleRow}>
                ...
                >
                  <Text
                    style={[
                      styles.pillText,
                      !compareOn && styles.pillTextActive,
                    ]}
                  >
                    Single
                  </Text>
            ...
            {compareOn ? (
              <View style={styles.entityPickers}>
                ...
                <TouchableOpacity
                  style={[
                    styles.entityCard,
                    searchTarget === "primary" && styles.entityCardFocused,
                  ]}
                  onPress={() => {
                    setSearchTarget("primary");
                ...
                >
                  <View style={styles.entityBadge}>
                    <View
                      style={[styles.dot, { backgroundColor: scout.playerA }]}
                    />
                  ...
                  <View style={styles.entityTextCol}>
                    ...
                    {primaryEntity.sub ? (
                      <Text style={styles.entitySub}>{primaryEntity.sub}</Text>
                    ) : null}
                  </View>
                </TouchableOpacity>
                ...
                <TouchableOpacity
                  style={[
                    styles.entityCard,
                    searchTarget === "secondary" && styles.entityCardFocused,
                  ]}
                  onPress={() => {
                ...
                >
                  <View style={styles.entityBadge}>
                    <View
                      style={[styles.dot, { backgroundColor: scout.playerB }]}
                    />
            ...
            <View style={styles.compareRow}>
              ...
              <View style={styles.toggleRow}>
                <TouchableOpacity
                  ...
                  style={[
                    styles.pill,
                    statBasis === "raw" && styles.activePill,
                  ]}
                >
                  <Text
                    style={[
                      styles.pillText,
                      statBasis === "raw" && styles.pillTextActive,
                    ]}
                  >
                    Raw
                  </Text>
                ...
                <TouchableOpacity
                  ...
                  style={[
                    styles.pill,
                    statBasis === "adjusted" && styles.activePill,
                  ]}
                >
                  <Text
                    style={[
                      styles.pillText,
                      statBasis === "adjusted" && styles.pillTextActive,
                    ]}
                  >
                    Per 100 poss.
                  </Text>
            ...
            >
              {availableStats.map((s) => (
                <TouchableOpacity
                  ...
                  style={[
                    styles.statBtn,
                    activeStat === s.value && styles.activeStat,
                  ]}
                ...
                >
                  <Text
                    style={[
                      styles.statBtnText,
                      activeStat === s.value && styles.statBtnTextActive,
                    ]}
                  >
                    {s.label}
                  </Text>
                ...
                <>
                  ...
                  chartResponse.seriesList.length >= 2 ? (
                    <>
                      ...
                      <Text style={styles.tapHint}>
                        Tap a point for stats, opponents, and final scores.
                      </Text>
                    ...
                    <>
                      ...
                      <Text style={styles.tapHint}>
                        Tap a point for game details and final score.
                      </Text>
                      <View style={styles.legendRow}>
                        <View style={styles.legendItem}>
                          <View
                  ...
                  {!compareOn &&
                    ...
                    singleSeriesPoints.length > 0 && (
                      ...
                      >
                        <View
                          style={[
                            styles.chartInteractiveWrap,
                            { width: chartWidth, minHeight: chartHeight + 24 },
                          ]}
                        >
                          <LineChart
                            ...
                            xAxisLabelsHeight={
                              singleChartHasAbsence ? 44 : undefined
                            }
                            rulesColor={sem.rules}
                            xAxisColor={sem.axis}
                            yAxisColor={sem.axis}
                  ...
                  {!compareOn && singleDetail ? (
                    <View
                      style={[
                        ...
                        {
                          backgroundColor: sem.insightBg,
                          borderColor: sem.insightBorder,
                        },
                      ]}
                    >
                      <View
                        style={[insightCompact.header, styles.insightHeaderRow]}
                      >
                        <Text
                          style={[insightCompact.title, { color: sem.text }]}
                        >
                          Game details
                        </Text>
                        <TouchableOpacity
                          onPress={() => setSingleSelectedIndex(null)}
                          hitSlop={12}
                        >
                          <Text
                            style={[
                              insightCompact.dismiss,
                      ...
                      <Text style={[insightCompact.date, { color: sem.muted }]}>
                        {singleDetail.isoDate
                          ? formatRangeDate(singleDetail.isoDate)
                          : "Selected game"}
                      </Text>
                      <Text
                        style={[
                          insightCompact.entity,
                          playerNameText,
                          { color: sem.text },
                        ]}
                      >
                      ...
                      <View style={styles.insightStatRow}>
                        ...
                        <Text
                          style={[
                            insightCompact.statValue,
                            { color: sem.text },
                          ]}
                        >
                          {typeof singleDetail.value === "number" &&
                          Number.isFinite(singleDetail.value)
                      ...
                      {singleDetail.chartAbsence ? (
                        <Text
                          style={[
                            insightCompact.body,
                            { color: sem.muted, marginBottom: 6 },
                          ]}
                        >
                      ...
                      Number.isFinite(singleDetail.trueShootingPct) ? (
                        <View
                          style={[
                            ...
                            {
                              backgroundColor: tsEfficiencyBadgeStyle(
                                singleDetail.trueShootingPct,
                              ).bg,
                            },
                          ]}
                        >
                          ...
                          >
                            Efficiency · TS%{" "}
                            {singleDetail.trueShootingPct.toFixed(1)}%
                          </Text>
                        </View>
                      ) : null}
                      <Text
                        style={[
                          insightCompact.sectionLabel,
                          { color: sem.label },
                        ]}
                      >
                        Opponent
                      ...
                      <Text style={[insightCompact.body, { color: sem.text }]}>
                        {singleDetail.opponentName?.trim() ||
                          "Opponent not available for this game."}
                      </Text>
                      <Text
                        style={[
                          insightCompact.sectionLabel,
                          { color: sem.label, marginTop: 6 },
                        ]}
                      >
                        Final score
                      </Text>
                      <Text style={[insightCompact.body, { color: sem.text }]}>
                        {singleDetail.gameScoreLine?.trim() ||
                  ...
                  {!compareOn && teamH2hDetail ? (
                    ...
                    >
                      ...
                      <Text style={[insightCompact.date, { color: sem.muted }]}>
                        {teamH2hDetail.a.isoDate
                          ? formatRangeDate(teamH2hDetail.a.isoDate)
                          : "Selected game"}
                      </Text>
                      <Text
                        style={[
                        ...
                        Number.isFinite(teamH2hDetail.a.trueShootingPct) ? (
                          <View
                            style={[
                              ...
                              {
                                backgroundColor: tsEfficiencyBadgeStyle(
                                  teamH2hDetail.a.trueShootingPct,
                                ).bg,
                              },
                            ]}
                          >
                            ...
                            >
                              A · TS%{" "}
                              {teamH2hDetail.a.trueShootingPct.toFixed(1)}%
                            </Text>
                          </View>
                        ) : null}
                        ...
                        Number.isFinite(teamH2hDetail.b.trueShootingPct) ? (
                          <View
                            style={[
                              ...
                              {
                                backgroundColor: tsEfficiencyBadgeStyle(
                                  teamH2hDetail.b.trueShootingPct,
                                ).bg,
                              },
                            ]}
                          >
                            ...
                            >
                              B · TS%{" "}
                              {teamH2hDetail.b.trueShootingPct.toFixed(1)}%
                            </Text>
                          </View>
                        ) : null}
                      </View>
                      <View
                        style={[
                          insightCompact.compareRow,
                          styles.insightCompareRowLayout,
                        ]}
                      >
                        <View
                          style={[
                            insightCompact.swatch,
                            { backgroundColor: scout.playerA },
                          ]}
                        />
                        <View style={styles.insightCompareMid}>
                          ...
                          <Text
                            style={[insightCompact.vs, { color: sem.muted }]}
                            numberOfLines={2}
                          >
                        ...
                        <View
                          style={[
                            insightCompact.statCol,
                            styles.insightStatRightCol,
                          ]}
                        >
                          <Text
                            style={[
                          ...
                          >
                            {typeof teamH2hDetail.a.value === "number" &&
                            Number.isFinite(teamH2hDetail.a.value)
                      ...
                      >
                        ...
                        >
                          ...
                          >
                            {typeof teamH2hDetail.b.value === "number" &&
                            Number.isFinite(teamH2hDetail.b.value)
                    ...
                    >
                      <View style={{ paddingVertical: 4 }}>
                        <LineChart
                          ...
                          xAxisLabelsHeight={
                            compareChartHasAbsence ? 44 : undefined
                          }
                          rulesColor={sem.rules}
                          nestedScrollEnabled
                        />
                  ...
                  {compareOn &&
                  compareDetail &&
                  chartResponse?.seriesList?.[0] &&
                  chartResponse.seriesList[1] ? (
                    <View
                      style={[
                        insightCompact.panel,
                        {
                    ...
                    >
                      ...
                      >
                        ...
                        <TouchableOpacity
                          onPress={() => setCompareSelectedIndex(null)}
                          hitSlop={12}
                        >
                          <Text
                            style={[
                      ...
                      >
                        {Array.from(
                          new Set(
                            [
                              compareDetail.scoreLineA,
                              compareDetail.scoreLineB,
                            ].filter((s): s is string => Boolean(s?.trim())),
                          ),
                        ).join(" · ") || "Score not available for this date."}
                      </Text>
                      <View style={styles.h2hTsBadgeRow}>
                        {typeof compareDetail.tsPctA === "number" &&
                        Number.isFinite(compareDetail.tsPctA) ? (
                          <View
                            style={[
                              styles.efficiencyBadge,
                              {
                                backgroundColor: tsEfficiencyBadgeStyle(
                                  compareDetail.tsPctA,
                                ).bg,
                              },
                            ]}
                          >
                        ...
                        {typeof compareDetail.tsPctB === "number" &&
                        Number.isFinite(compareDetail.tsPctB) ? (
                          <View
                            style={[
                              styles.efficiencyBadge,
                              {
                                backgroundColor: tsEfficiencyBadgeStyle(
                                  compareDetail.tsPctB,
                                ).bg,
                              },
                            ]}
                          >
                      ...
                      >
                        ...
                        >
                          ...
                          >
                            {compareDetail.valueA !== undefined
                              ? compareDetail.valueA.toFixed(1)
                              : "—"}
                          </Text>
                        </View>
                      </View>
                      ...
                      >
                        ...
                        >
                          ...
                          >
                            {compareDetail.valueB !== undefined
                              ? compareDetail.valueB.toFixed(1)
                              : "—"}
                          </Text>
                        </View>
                      </View>
                  ...
                  !lastResponse ? (
                    <Text style={styles.emptyState}>
                      Select a {mode} and stat to load the chart.
                    </Text>
                  ) : null}
                  ...
                  !dataError ? (
                    <Text style={styles.emptyState}>
                      Not enough games in range for both sides to compare (each
                      needs at least one game).
                    </Text>
                  ) : null}
                </>
              )}
        ...
        >
          ...
          >
            ...
            <Text
              style={[styles.sheetSectionTitle, styles.sheetSectionTitleSpaced]}
            >
              Chart filters
            </Text>
            ...
            <View style={styles.filterRow}>
              <TouchableOpacity
                ...
                onPress={() => applyPresetRange("pastMonth")}
              >
                <Text style={styles.presetBtnText}>Past month</Text>
              </TouchableOpacity>
              <TouchableOpacity
                style={styles.presetBtn}
                onPress={() => applyPresetRange("past3Months")}
              >
                <Text style={styles.presetBtnText}>Past 3 mo</Text>
              </TouchableOpacity>
              <TouchableOpacity
                style={styles.presetBtn}
                onPress={() => applyPresetRange("thisSeason")}
              >
            ...
            <View style={styles.filterMetaRow}>
              <Text style={styles.filterMetaText}>
                Default line (avg): {averageLineValue.toFixed(1)}{" "}
                {chartStatLabel}
              </Text>
              <TouchableOpacity
                style={[
                  styles.toggleBtn,
                  showReferenceLine && styles.toggleBtnActive,
                ]}
              ...
              >
                <Text
                  style={[
                    ...
                    showReferenceLine && styles.toggleBtnTextActive,
                  ]}
                >
              ...
              <View style={styles.opponentFilterBlock}>
                ...
                {opponentFilterTeam ? (
                  <View style={styles.opponentPillRow}>
                    <Text style={styles.opponentPillText}>
                      vs {opponentFilterTeam.name}
                    </Text>
                    <TouchableOpacity
                      onPress={() => {
                        setOpponentFilterTeam(null);
