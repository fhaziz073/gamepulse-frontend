...
import { absenceMarkerForLineItem, type AbsenceAxisColors } from './chartAbsenceMarkers';
...
const sortChronologically = (points: ChartPointInput[]): ChartPointInput[] => {
  ...
  if (points.every((p) => p.isoDate)) {
    return [...points].sort((x, y) => String(x.isoDate).localeCompare(String(y.isoDate)));
  }
  return [...points];
};
...
export function alignTwoSeriesForComparison(
  ...
  for (const iso of dates) {
    ...
    rows.push({
      ...
      isoDate: iso,
      valueA:
        pa && pa.value !== null && typeof pa.value === 'number' && Number.isFinite(pa.value)
          ? pa.value
          : undefined,
      valueB:
        pb && pb.value !== null && typeof pb.value === 'number' && Number.isFinite(pb.value)
          ? pb.value
          : undefined,
