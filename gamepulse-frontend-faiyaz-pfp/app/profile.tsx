...
function mergeUserPreference(userInfo: any, prefPatch: Record<string, unknown>) {
...
export default function ProfileScreen() {
  ...
  const userId: string | null = (userInfo?.id ?? userInfo?.["User ID"] ?? null) as
    | string
    | null;
  const currentUsername: string = (userInfo?.username ??
    userInfo?.Username ??
    "") as string;
  const currentEmail: string = (userInfo?.email ?? userInfo?.Email ?? "") as string;
  ...
  const [currentPasswordForRefresh, setCurrentPasswordForRefresh] = useState("");
  ...
  const onSaveAccount = async () => {
    ...
    if (trimmedUsername && trimmedUsername !== currentUsername) {
      actions.push(patchJson("/users/username", { userID: id, newUsername: trimmedUsername }));
    }
    if (trimmedEmail && trimmedEmail !== currentEmail) {
      actions.push(patchJson("/users/email", { userID: id, newEmail: trimmedEmail }));
    }
    if (trimmedAvatar !== currentAvatarUrl) {
      actions.push(patchJson("/users/avatarUrl", { userID: id, newAvatarUrl: trimmedAvatar }));
    }
    if (newPassword.trim()) {
      actions.push(patchJson("/users/password", { userID: id, newPassword: newPassword.trim() }));
    }
    ...
    if (failed.length) {
      ...
      Alert.alert("Save failed", `Server returned ${first.status}.\n\n${msg || "No details."}`);
      return;
    }
    ...
    dispatch(
      setUserInfo(
        mergeUserAccount(userInfo, {
          ...(trimmedUsername ? { username: trimmedUsername, Username: trimmedUsername } : {}),
          ...(trimmedEmail ? { email: trimmedEmail, Email: trimmedEmail } : {}),
          ...(trimmedAvatar ? { avatarUrl: trimmedAvatar, "Avatar URL": trimmedAvatar } : {}),
        }),
      ),
    );
  ...
  const onSaveNotifications = async () => {
    ...
    const res1 = await patchJson("/users/pref/gs", { userID: id, newGS: gameStartNotifPref });
    if (!res1.ok) {
      Alert.alert("Save failed", `Games Starting pref: ${res1.status}`);
      return;
    }

    const res2 = await patchJson("/users/pref/ogc", { userID: id, newOGC: ongoingGameNotifPref });
    if (!res2.ok) {
  ...
  const onSaveTeams = async () => {
    ...
    const res = await patchJson("/users/pref/teams", { userID: id, newTeams: selectedTeamIds });
    if (!res.ok) {
    ...
    <View style={styles.page}>
      <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        ...
        >
          ...
          <Text style={styles.toggleValue}>{gameStartNotifPref ? "On" : "Off"}</Text>
        </Pressable>
        <Pressable
          style={styles.toggleRow}
        ...
        >
          ...
          <Text style={styles.toggleValue}>{ongoingGameNotifPref ? "On" : "Off"}</Text>
        </Pressable>
        ...
        <FlatList
          ...
          renderItem={({ item }) => {
            ...
            return (
              <Pressable style={styles.teamRow} onPress={() => toggleTeam(item.id)}>
                <Text style={styles.teamName}>{item.name}</Text>
                <Text style={styles.teamSelected}>{isSelected ? "Selected" : ""}</Text>
              </Pressable>
            );
          }}
        />
